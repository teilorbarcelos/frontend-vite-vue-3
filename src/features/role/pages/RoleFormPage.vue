...
watch(
  ...
  ([newFeatures, newRole]) => {
    ...
    const initialPermissions = newFeatures.map((feature) => {
      const existing = newRole?.RoleFeature?.find(
        (rf: RoleFeature) => rf.id_feature === feature.id
      );
      return {
...
<template>
  <div
    v-if="(isEditing && isLoadingRole) || isLoadingFeatures"
  ...
  <div v-else class="overflow-y-auto flex-1 pb-8">
    ...
    >
      <div class="flex items-center justify-between border-b border-gray-200 pb-4">
        ...
        <Button variant="ghost" @click="router.push('/roles')"> Cancelar </Button>
      </div>
                ...
                >
                  <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="text-xs text-gray-500">
                      {{
                        features?.find((f) => f.id === values.permissions[index].id_feature)
                          ?.description
                      }}
                    </div>
                  </td>
