...
<template>
  <div v-if="showPagination" class="flex flex-col gap-4 px-4 py-4 bg-white border-t border-gray-100 sm:px-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div v-if="onPageSizeChange && pageSize !== undefined" class="flex items-center space-x-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger as-child>
              <button class="flex items-center space-x-2 text-sm font-medium text-gray-700 bg-gray-50/50 border border-gray-200 rounded-lg px-3 py-1.5 hover:bg-white hover:border-gray-300 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                <span>{{ pageSize }}</span>
